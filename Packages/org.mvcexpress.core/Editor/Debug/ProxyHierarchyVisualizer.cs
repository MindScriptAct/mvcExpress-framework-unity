...
{
    /// <summary>
    /// Play-mode-only hook that mirrors registered proxies as child GameObjects under each module's (or the global) hierarchy root, purely for inspecting live proxy state in the Hierarchy window.
    /// </summary>
    [InitializeOnLoad]
    internal static class ProxyHierarchyVisualizer
    {
        ...
        // Mirrors the above for ProxyUnregisteredForDebug - lets us remove a wrapper GO directly
        // instead of MvcDiContainer.UnregisterInternal scanning every MvcModule in the scene - see L11.
        private static readonly Dictionary<MvcDiContainer, Action<object>> _unregisterHandlers = new Dictionary<MvcDiContainer, Action<object>>();
        private static readonly EventInfo _proxyUnregisteredEvent = typeof(MvcDiContainer).GetEvent(
            "ProxyUnregisteredForDebug",
            BindingFlags.Instance | BindingFlags.NonPublic);

        private static MethodInfo AddUnregisterHandlerMethod => _proxyUnregisteredEvent?.GetAddMethod(true);
        private static MethodInfo RemoveUnregisterHandlerMethod => _proxyUnregisteredEvent?.GetRemoveMethod(true);

        // Remembers where each container's wrapper GOs live (module transform + "Model", or the
        // MvcFacade transform + "Static") so the unregister handler can look them up without
        // creating a Model root that was never needed.
        private static readonly Dictionary<MvcDiContainer, (Transform Owner, string RootName)> _ownerRoots =
            new Dictionary<MvcDiContainer, (Transform, string)>();
...
#pragma warning restore CS0618
        ...
        private static void HookGlobal()
        {
            ...
            _ownerRoots[container] = (app.transform, GlobalModelRootName);
            Action<object> unregisterHandler = proxy => OnProxyUnregistered(container, proxy);
            if (_proxyUnregisteredEvent != null && AddUnregisterHandlerMethod != null)
            {
                AddUnregisterHandlerMethod.Invoke(container, new object[] { unregisterHandler });
                _unregisterHandlers[container] = unregisterHandler;
            }
        ...
        private static void UnhookAll()
        {
            ...
            if (_proxyUnregisteredEvent == null || RemoveUnregisterHandlerMethod == null)
            {
                _unregisterHandlers.Clear();
            }
            else
            {
                foreach (var kvp in _unregisterHandlers)
                {
                    RemoveUnregisterHandlerMethod.Invoke(kvp.Key, new object[] { kvp.Value });
                }
                _unregisterHandlers.Clear();
            }
        ...
        private static void HookModule(MvcModule module)
        {
            ...
            _ownerRoots[container] = (module.transform, ModuleModelRootName);
            if (_proxyUnregisteredEvent != null && AddUnregisterHandlerMethod != null)
            {
                Action<object> unregisterHandler = proxy => OnProxyUnregistered(container, proxy);
                AddUnregisterHandlerMethod.Invoke(container, new object[] { unregisterHandler });
        ...
        // Removes the wrapper GO for a code-only proxy that was just unregistered from `container`.
        // Looks up the owner root recorded in HookModule/HookGlobal instead of creating one (a
        // container that never had any proxies registered has no Model root to search) - see L11.
        private static void OnProxyUnregistered(MvcDiContainer container, object proxy)
        {
            if (proxy == null) return;
            if (proxy is ProxyBehaviour) return; // GameObject lifecycle handled by UnregisterInternal directly
            if (!_ownerRoots.TryGetValue(container, out var ownerRoot) || ownerRoot.Owner == null) return;

            var modelRoot = ownerRoot.Owner.Find(ownerRoot.RootName);
            if (modelRoot == null) return;

            var wrapper = modelRoot.Find(proxy.GetType().Name);
            if (wrapper == null) return;

            if (wrapper.GetComponent("ProxyDebugBehaviour") != null)
            {
        ...
        private static void EnsureCodeProxyGO(Transform modelRoot, object proxy)
        {
            ...
            if (dbg == null)
            {
                var t = Type.GetType("mvcExpress.Internal.ProxyDebug.ProxyDebugBehaviour, org.mvcexpress.core");
                if (t != null)
                {
