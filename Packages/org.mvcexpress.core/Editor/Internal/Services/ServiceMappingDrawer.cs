...
namespace mvcExpress.Editor.Core
{
    ...
    public sealed class ServiceMappingDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            // Permanent/Transient Badge (click to toggle)
            var lifecycleBadgeRect = new Rect(currentX + CHECKBOX_PADDING / 2, rowRect.y, CHECKBOX_WIDTH, rowRect.height);
            bool newTransientValue = MvcEditorUtility.DrawLifecycleBadge(lifecycleBadgeRect, isTransientProp.boolValue, "Service");
            if (newTransientValue != isTransientProp.boolValue)
            {
