...
namespace mvcExpress
{
    ...
    public partial class MediatorBehaviour : MonoBehaviour
    {
        ...
        // Concrete type, not IMessageBus: MvcMessageBus is sealed and the only implementation,
        // and this field backs every Publish/Subscribe call a mediator makes - storing the
        // interface here would force interface dispatch (and defeat AggressiveInlining/IL2CPP
        ...
        public void Initialize(MvcModule module, MvcDiContainer diContainer, IMessageBus messageBus, bool deferOnInitialized = false)
        {
            ...
            // Public signature keeps accepting the interface for API stability; the concrete
            // type is recovered once here (MvcMessageBus is the only implementation) so every
            // subsequent Publish/Subscribe call through this._messageBus can be inlined/devirtualized.
            this._messageBus = (MvcMessageBus)messageBus;
