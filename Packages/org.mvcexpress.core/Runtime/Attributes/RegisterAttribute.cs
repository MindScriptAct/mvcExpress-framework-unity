...
namespace mvcExpress
{
    ...
    public sealed class RegisterAttribute : Attribute
    {
        ...
        /// Permanent: Lives for the entire module lifetime (default, recommended for most cases). Cannot be unregistered.
        /// Transient: Can be destroyed/unregistered dynamically (use when lifecycle is tied to runtime state).
        /// Scoped: Fresh instance per resolution scope. Only valid for non-MonoBehaviour types (plain Proxy or
        /// plain Service classes) - throws at scan time if applied to a ProxyBehaviour or MonoBehaviour service.
        /// Default: Permanent
        /// </summary>
        ...
        /// <remarks>
        /// <see cref="RegisterToLogic"/> defaults to <c>true</c> and <see cref="Lifecycle"/> defaults to
        /// <see cref="RegistrationLifecycle.Permanent"/>, so <c>[Register(typeof(MyModule))]</c> alone is
        /// exactly equivalent to the fully explicit form below - the shorthand and the full form register
        /// identically.
        /// </remarks>
        /// Full Example :
        ///     [Register(typeof(MyModule), RegisterToLogic = true, RegisterToView = true, LogicInterface = typeof(IMyProxy), ViewInterface = typeof(IMyProxy), Lifecycle = RegistrationLifecycle.Permanent) ]
        ...
        internal void Validate(Type targetType)
        {
            ...
            {
                throw new InvalidOperationException(
                    $"[Register] Type '{targetType.FullName}' cannot use Lifecycle.Scoped because it is a MonoBehaviour. " +
