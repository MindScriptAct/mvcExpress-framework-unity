...
namespace mvcExpress.Logging
{
    ...
    internal static class MvcLogInternal
    {
        ...
        /// <summary>
        /// Log that an async command's execution was cancelled - an expected outcome of its
        /// module being destroyed mid-operation, not a failure. Must not use MvcDebug.LogError.
        /// </summary>
        [Conditional("UNITY_EDITOR"), Conditional("MVC_LOGGING")]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void LogCommandCancelled<TCommand>(mvcExpress.MvcModule module)
        {
            if (!ShouldLog(module)) return;
            ...
            var message = $"[{commandTypeName}] cancelled (module destroyed)";
        ...
        // Exposes ShouldLog to callers that need to gate expensive diagnostic work (e.g. a
        // StackTrace capture) before even reaching a Log* call, not just the log line itself -
        // see L5.
        internal static bool ShouldLogFor(mvcExpress.MvcModule module) => ShouldLog(module);
