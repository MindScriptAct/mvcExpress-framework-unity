...
namespace mvcExpress.Internal.Commands
{
    public sealed partial class MvcCommandProcessor
    {
        ...
        // Detects an async command type so BindCommand/Run can route to the async execution path
        // automatically, without requiring callers to call BindCommandAsync/RunAsync explicitly.
        ...
        /// <summary>
        /// Binds a sync command to a parameter-less message. The command executor is registered
        /// directly as a message handler - zero overhead beyond the delegate call itself.
        /// </summary>
            ...
            where TMessage : IMessage
        {
            ...
            ValidateCommandArity<TCommand, Command>();
        ...
        public void BindCommandAsync<TCommand, TMessage>(uint poolSize = 0)
            ...
            where TMessage : IMessage
        {
            ValidateCommandArity<TCommand, CommandAsync>();
