...
namespace mvcExpress.Internal.Messaging
{
    public sealed partial class MvcMessageBus : IMessageBus, IDisposable
    {
        ...
        /// <summary>
        /// Publishes a no-payload message to every subscriber on this bus instance, in subscribe order.
        ...
        /// <summary>
        /// Subscribes a handler to a no-payload message on this bus instance.
        /// </summary>
        ...
        private SubscriptionToken SubscribeInternal<TMessage>(Action handler) where TMessage : IMessage
        {
            ...
            var freeIndex = Storage0<TMessage>.InstanceFreeListHeads[_instanceId];
            if (freeIndex != NO_FREE_SLOT)
            {
                Storage0<TMessage>.InstanceFreeListHeads[_instanceId] = DecodeNextFree(versions[freeIndex]);

                var reusedVersion = ++Storage0<TMessage>.InstanceVersionCounters[_instanceId];
                handlers[freeIndex] = handler;
                versions[freeIndex] = reusedVersion;

                var reusedToken = new SubscriptionToken(freeIndex, reusedVersion);
#if UNITY_EDITOR || DEVELOPMENT_BUILD
                reusedToken.DebugName = typeof(TMessage).Name;
        ...
        public void Unsubscribe<TMessage>(SubscriptionToken token) where TMessage : IMessage
        {
            ...
            if (idx >= 0 && idx < Storage0<TMessage>.InstanceCounts[_instanceId])
            {
                if (Storage0<TMessage>.InstanceVersions[_instanceId][idx] == token.Version)
                {
                    ...
                    Storage0<TMessage>.InstanceVersions[_instanceId][idx] = EncodeNextFree(Storage0<TMessage>.InstanceFreeListHeads[_instanceId]);
                    Storage0<TMessage>.InstanceFreeListHeads[_instanceId] = idx;
                }
            }
        }
        ...
        public void Unsubscribe<TMessage>(Action handler) where TMessage : IMessage
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    handlers[i] = null;
                    Storage0<TMessage>.InstanceVersions[_instanceId][i] = EncodeNextFree(Storage0<TMessage>.InstanceFreeListHeads[_instanceId]);
                    Storage0<TMessage>.InstanceFreeListHeads[_instanceId] = i;
                    return;
                }
            }
        }
        ...
        /// <summary>
        /// Removes every subscriber for a no-payload message on this bus instance, and resets the
        ...
        public void UnsubscribeAll<TMessage>() where TMessage : IMessage
        {
            ...
            Storage0<TMessage>.InstanceFreeListHeads[_instanceId] = NO_FREE_SLOT;
        }
