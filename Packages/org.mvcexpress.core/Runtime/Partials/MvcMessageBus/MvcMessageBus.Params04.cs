...
namespace mvcExpress.Internal.Messaging
{
    public sealed partial class MvcMessageBus : IMessageBus, IDisposable
    {
        ...
        /// <summary>
        /// Publishes a four-payload message to every subscriber on this bus instance, in subscribe order.
        ...
        /// <summary>
        /// Subscribes a handler to a four-payload message on this bus instance.
        /// </summary>
        ...
        private SubscriptionToken SubscribeInternal<TMessage, T1, T2, T3, T4>(Action<T1, T2, T3, T4> handler) where TMessage : IMessage<T1, T2, T3, T4>
        {
            ...
            var freeIndex = Storage4<TMessage, T1, T2, T3, T4>.InstanceFreeListHeads[_instanceId];
            if (freeIndex != NO_FREE_SLOT)
            {
                Storage4<TMessage, T1, T2, T3, T4>.InstanceFreeListHeads[_instanceId] = DecodeNextFree(versions[freeIndex]);

                var reusedVersion = ++Storage4<TMessage, T1, T2, T3, T4>.InstanceVersionCounters[_instanceId];
                handlers[freeIndex] = handler;
                versions[freeIndex] = reusedVersion;

                var reusedToken = new SubscriptionToken(freeIndex, reusedVersion);
#if UNITY_EDITOR || DEVELOPMENT_BUILD
                reusedToken.DebugName = $"{typeof(TMessage).Name}<{typeof(T1).Name},{typeof(T2).Name},{typeof(T3).Name},{typeof(T4).Name}>";
        ...
        public void Unsubscribe<TMessage, T1, T2, T3, T4>(SubscriptionToken token) where TMessage : IMessage<T1, T2, T3, T4>
        {
            ...
            if (idx >= 0 && idx < Storage4<TMessage, T1, T2, T3, T4>.InstanceCounts[_instanceId])
            {
                if (Storage4<TMessage, T1, T2, T3, T4>.InstanceVersions[_instanceId][idx] == token.Version)
                {
                    ...
                    Storage4<TMessage, T1, T2, T3, T4>.InstanceVersions[_instanceId][idx] = EncodeNextFree(Storage4<TMessage, T1, T2, T3, T4>.InstanceFreeListHeads[_instanceId]);
                    Storage4<TMessage, T1, T2, T3, T4>.InstanceFreeListHeads[_instanceId] = idx;
                }
            }
        }
        ...
        public void Unsubscribe<TMessage, T1, T2, T3, T4>(Action<T1, T2, T3, T4> handler) where TMessage : IMessage<T1, T2, T3, T4>
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    handlers[i] = null;
                    Storage4<TMessage, T1, T2, T3, T4>.InstanceVersions[_instanceId][i] = EncodeNextFree(Storage4<TMessage, T1, T2, T3, T4>.InstanceFreeListHeads[_instanceId]);
                    Storage4<TMessage, T1, T2, T3, T4>.InstanceFreeListHeads[_instanceId] = i;
                    return;
                }
            }
        }
        ...
        /// <summary>
        /// Removes every subscriber for a four-payload message on this bus instance, and resets the
        ...
        public void UnsubscribeAll<TMessage, T1, T2, T3, T4>() where TMessage : IMessage<T1, T2, T3, T4>
        {
            ...
            Storage4<TMessage, T1, T2, T3, T4>.InstanceFreeListHeads[_instanceId] = NO_FREE_SLOT;
        }
        ...
        /// </summary>
        /// <summary>
        /// Returns whether a four-payload message has ever had a subscriber on this bus instance that has
        ...
        public bool HasSubscribers<TMessage, T1, T2, T3, T4>() where TMessage : IMessage<T1, T2, T3, T4>
        {
            ...
            return Storage4<TMessage, T1, T2, T3, T4>.InstanceCounts[_instanceId] > 0;
        }
