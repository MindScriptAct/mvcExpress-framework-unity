...
namespace mvcExpress.Tests
{
    public class MvcDiContainerTests
    {
        ...
        private class MockMonoBehaviourService : UnityEngine.MonoBehaviour {}
        ...
        public void Register_ToSameScopeTwice_ThrowsInvalidOperationException()
        {
            ...
            _container.Register(service).ToLogic().AsPermanent();

            var ex = Assert.Throws<InvalidOperationException>(() => 
            {
                _container.Register(new MockService()).ToLogic().AsPermanent();
            });
        ...
        {
            var service = new MockService();
            _container.Register(service).ToView().AsPermanent();
        ...
        public void ToLogicList_ResolvesAsList()
        {
            ...
            _container.Register<IMockService>(service1).ToLogicList<IMockService>().AsPermanent();
            _container.Register<IMockService>(service2).ToLogicList<IMockService>().AsPermanent();
        ...
        [Test]
        public void Unregister_PermanentType_Throws()
        {
            var service = new MockService();
            _container.Register(service).ToLogic().AsPermanent();
            ...
            Assert.Throws<InvalidOperationException>(
                () => _container.Unregister<MockService>(),
                "Permanent types must not be unregisterable.");
        }
        ...
        public void RegistrationBuilder_NoLayerSelected_ThrowsBeforeComplete()
        {
            ...
            // AsPermanent without ToLogic/ToView must throw
            Assert.Throws<InvalidOperationException>(() => builder.AsPermanent(),
        ...
        public void RegistrationBuilder_AlreadyCompleted_ThrowsOnFurtherCalls()
        {
            ...
            builder.AsPermanent(); // completes the builder
        ...
        {
            var service = new MockService();
            _container.Register(service).ToLogicAs<IMockService>().ToView().AsPermanent();
        ...
        public void RegistrationBuilder_DualRegistrationFails_RollsBackLogic()
        {
            ...
            _container.Register(existing).ToView().AsPermanent();
            ...
            Assert.Throws<InvalidOperationException>(
                () => _container.Register(second).ToLogic().ToView().AsPermanent(),
        ...
        [Test]
        public void RegistrationBuilder_AsScopedOnPlainNonProxyService_Succeeds()
        {
            // Plain POCO services (not extending Proxy, not a MonoBehaviour) can be Scoped -
            // the container only rejects MonoBehaviour instances, since it must be able to
            // construct the instance itself via Activator.CreateInstance.
            var service = new MockService();

            Assert.DoesNotThrow(() => _container.Register(service).ToLogic().AsScoped());
            Assert.That(_container.IsScoped(typeof(MockService)), Is.True);
        }

        [Test]
        public void RegistrationBuilder_AsScopedOnMonoBehaviour_ThrowsInvalidOperationException()
        {
            var go = new UnityEngine.GameObject("MockMonoBehaviourService");
            try
            {
                var service = go.AddComponent<MockMonoBehaviourService>();

                var ex = Assert.Throws<InvalidOperationException>(
                    () => _container.Register(service).ToLogic().AsScoped());

                Assert.That(ex.Message, Does.Contain("AsScoped() is not supported for MonoBehaviour"));
            }
            finally
            {
        ...
        {
            _container.Register(new ScopedProxy()).ToLogic().AsPermanent();
