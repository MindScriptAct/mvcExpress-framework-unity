...
namespace mvcExpress.Tests.Scenarios
{
    ...
    public class EdgeCaseScenarioTests
    {
        ...
        {
            _container.Register(new ScenarioService()).ToLogic().AsPermanent();
        ...
        public void DuplicateRegistration_RollbackOnFailure_ContainerStateUnchanged()
        {
            ...
            _container.Register(original).ToView().AsPermanent();

            var replacement = new ScenarioService();
            Assert.Throws<InvalidOperationException>(() =>
                _container.Register(replacement).ToLogic().ToView().AsPermanent(),
            ...
            Assert.DoesNotThrow(() => _container.Register(new AnotherScenarioService()).ToLogic().AsPermanent(),
        ...
        public void GlobalContainerLifecycle_FacadeDestroyed_GlobalActorsCleanedUp()
        {
            ...
            var service = new ScenarioService();
            globalContainer.Register(service).ToLogic().AsPermanent();

            globalContainer.Dispose();

            Assert.That(service.Disposed, Is.True,
                "Disposing the facade-owned global container should dispose permanent global service instances.");
        }
        ...
        {
            var globalContainer = new MvcDiContainer();
            globalContainer.Register(new AnotherScenarioService()).ToLogic().AsPermanent();
            _container.Register(new ScenarioService()).ToLogic().AsPermanent();
