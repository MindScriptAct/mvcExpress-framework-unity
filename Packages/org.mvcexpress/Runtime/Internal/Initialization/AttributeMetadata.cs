...
namespace mvcExpress.Internal.Initialization
{
    ...
    internal sealed class ProxyRegistrationMetadata
    {
        ...
        public ProxyRegistrationMetadata(Type proxyType, RegisterAttribute attribute)
        {
            ...
            IsTransient = attribute.Lifecycle == RegistrationLifecycle.Transient;
    ...
    /// <summary>
    /// Immutable snapshot of a <c>[RegisterGlobal]</c> attribute applied to a plain C# class
    /// (non-MonoBehaviour), captured once during assembly scanning.
    /// </summary>
    /// <remarks>
    /// MonoBehaviour types are rejected at scan time; only plain C# <see cref="Proxy"/> and
    /// non-MonoBehaviour service classes are valid targets.
    /// Validation is performed eagerly in the constructor (fail-fast during scan).
    /// Internal - consumed only by <see cref="MvcFacade"/>.
    /// </remarks>
    internal sealed class GlobalRegistrationMetadata
    {
        /// <summary>Concrete type to instantiate and register.</summary>
        ...
        /// <summary>True when the concrete type inherits from <see cref="Proxy"/>.</summary>
        ...
        public GlobalRegistrationMetadata(Type concreteType, RegisterGlobalAttribute attr)
        {
            ...
            IsProxy = typeof(mvcExpress.Proxy).IsAssignableFrom(concreteType);
