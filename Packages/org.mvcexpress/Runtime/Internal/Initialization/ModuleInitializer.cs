    ...
    internal sealed class ModuleInitializer
    {
        ...
        private void RegisterAttributeProxies()
        {
            ...
            foreach (var metadata in proxyMetadata)
            {
                ...
                {
                    // ProxyBehaviour: Must exist in Unity registry
                    if (trackedProxies != null)
                    {
                        foreach (var tracked in trackedProxies)
                        {
                            ...
                            {
                                proxyInstance = tracked;
                                break;
                            }
                        }
                    }

                    // Skip if no instance found (ProxyBehaviour must be in scene)
                    if (proxyInstance == null)
                    {
#if UNITY_EDITOR || MVC_LOGGING
                        MvcDebug.LogWarning(
                            $"Proxy '{metadata.ProxyType.FullName}' is a ProxyBehaviour with [Register] attribute. " +
                            $"ProxyBehaviour proxies must exist in the scene and be registered via Unity registry. " +
                            $"Skipping attribute registration for module '{_moduleType}'.");
#endif
                        continue;
                    }
                }
        ...
        private void RegisterAttributeServices()
        {
            ...
            foreach (var metadata in serviceMetadata)
            {
                try
                {
                    // Create instance of the service (non-MonoBehaviour services only)
                    // MonoBehaviour services must exist in the scene (Unity registry)
                    object serviceInstance = null;

                    if (typeof(UnityEngine.MonoBehaviour).IsAssignableFrom(metadata.ServiceType))
                    {
                        // MonoBehaviour services must be in the scene - skip attribute registration
                        // They should be registered via Unity registry (ServiceRegistry)
#if UNITY_EDITOR || MVC_LOGGING
                        MvcDebug.LogWarning(
                            $"Service '{metadata.ServiceType.FullName}' is a MonoBehaviour with [Register] attribute. " +
                            $"MonoBehaviour services must exist in the scene and be registered via Unity registry. " +
                            $"Skipping attribute registration for module '{_moduleType}'.");
#endif
                        continue;
                    }
        ...
        private void RegisterSerializedCommandBindings()
        {
            ...
            for (int i = 0; i < bindings.Length; i++)
            {
                ...
                try
                {
                    // Async is now inferred from command type.
                    var isAsync = typeof(CommandAsync).IsAssignableFrom(cmdType);
