...
namespace mvcExpress.Internal.Messaging
{
    ...
    public sealed class SubscriptionTracker
    {
        ...
        public bool Untrack(SubscriptionToken token)
        {
            lock (_lock)
            {
                for (int i = _subscriptions.Count - 1; i >= 0; i--)
                {
                    if (_subscriptions[i].Token.Index == token.Index &&
        ...
        /// the counter reaches <c>CleanupThreshold</c>.
        /// Call this periodically (e.g. each time a new subscription is tracked) to lazily
        /// reclaim slots from GC'd subscribers without paying the full scan cost every time.
        /// </summary>
